...
export function VoiceInput({ onAudioSend, disabled }: VoiceInputProps) {
    ...
    useEffect(() => {
        if (typeof navigator !== 'undefined' && navigator.permissions && (navigator.permissions as unknown as { query: unknown }).query) {
            (navigator.permissions as unknown as { query: (options: { name: string }) => Promise<{ state: string, onchange: (() => void) | null }> }).query({ name: 'microphone' }).then((result: { state: string, onchange: (() => void) | null }) => {
    ...
    const startRecording = async () => {
        ...
        if (navigator.permissions && (navigator.permissions as unknown as { query: unknown }).query) {
            try {
                const result = await (navigator.permissions as unknown as { query: (options: { name: string }) => Promise<{ state: string, onchange: (() => void) | null }> }).query({ name: 'microphone' });
        ...
        try {
            ...
            setIsProcessing(false)
            setLastError(err instanceof Error ? err.name : "Unknown Error")

            const errName = err instanceof Error ? err.name : "Unknown Error";
            if (errName === 'NotAllowedError' || errName === 'PermissionDeniedError' || errName === 'NotSecureContext') {
                setShowPermissionPrompt(true)
                ...
                alert(`Microphone error (${errName}): ${err instanceof Error ? err.message : String(err)}`)
            }
        }
    }
