...
export function VoiceInput({ onAudioSend, disabled }: VoiceInputProps) {
    ...
    const startRecording = async () => {
        ...
        if (navigator.permissions && (navigator.permissions.query as (descriptor: PermissionDescriptor) => Promise<PermissionStatus>)) {
            try {
                const result = await (navigator.permissions.query as (descriptor: PermissionDescriptor) => Promise<PermissionStatus>)({ name: 'microphone' as PermissionName });
        ...
        try {
            ...
            console.error("Mic Access Error:", err instanceof Error ? err.name : "Unknown", err instanceof Error ? err.message : String(err))
            setIsProcessing(false)
            setLastError(err instanceof Error ? err.name : "UnknownError")

            if ((err instanceof Error ? err.name : '') === 'NotAllowedError' || (err instanceof Error ? err.name : '') === 'PermissionDeniedError' || (err instanceof Error ? err.name : '') === 'NotSecureContext') {
                setShowPermissionPrompt(true)
            } else if ((err instanceof Error ? err.name : '') === 'NotFoundError' || (err instanceof Error ? err.name : '') === 'DevicesNotFoundError') {
                alert("No microphone found. Please connect a microphone and try again.")
            } else if ((err instanceof Error ? err.name : '') === 'NotReadableError' || (err instanceof Error ? err.name : '') === 'TrackStartError') {
                alert("Your microphone is currently being used by another application.")
            } else {
                alert(`Microphone error (${err instanceof Error ? err.name : 'Unknown'}): ${err instanceof Error ? err.message : String(err)}`)
            }
        }
    }
